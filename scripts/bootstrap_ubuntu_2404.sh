...
prompt_required() {
  ...
  if [[ -t 0 ]]; then
    ...
    if [[ -z "$current" ]]; then echo "$var_name is required" >&2; exit 1; fi
    printf -v "$var_name" '%s' "$current"
  else
    echo "$var_name is required. Example: curl -fsSL https://raw.githubusercontent.com/synapticloop/digitalocean-wireguard-vpn/main/install.sh | sudo WG_ADMIN_DOMAIN=wg.example.com bash" >&2
    exit 1
  fi
}
...
prompt_required WG_ADMIN_DOMAIN "Admin domain, e.g. wg.example.com: "
...
apt-get update
apt-get install -y wireguard wireguard-tools python3 python3-venv python3-pip git nginx certbot ufw sqlite3 curl dnsutils qrencode
